...
class HomeLogged extends Component {
  ...
  userList = props => {
    ...
    console.log(value)
    if (this.props.userConnectedData.packageId ===null|| this.props.userConnectedData.packageName  === null) {
      // Limit to 5 cards if the packageId is null
    ...
    if (value.length !== 0) {
      const users = value.map((e, index) => (
        <UserCard
          ...
          func={this.sendNotif}
          key={index}
          pictures={this.props.userConnectedData.pictures.length > 0 ? true : false}
        />
      ));
      return <ul>{users}</ul>;
    }
