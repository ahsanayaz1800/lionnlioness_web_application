...
findUsersByName: async (name) => {
  try {
      ...
      // Create dynamic SQL based on the number of name parts
 ...
 // Update user method with proper error handling and promise resolution
   ...
   updateUser: (userId, updateData) => {
    ...
    // Dynamically building query based on provided fields
    ...
    console.log('Update query:', query); // Debugging the query
    console.log('Update values:', values); // Debugging the values

    return new Promise((resolve, reject) => {
        pool.query(query, values, (error, result) => {
            if (error) {
                console.error('Database update error:', error); // Log actual error
                return reject(error);
            }
            console.log('Update result:', result); // Log result to check affectedRows
            resolve(result);
        });
    });
},
